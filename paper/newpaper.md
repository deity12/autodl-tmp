...
为了全面评估模型性能，本文采用三类评价指标：

**1. 统计误差类（证明"拟合"得好）**：
- **MSE (均方误差)**：训练过程的主要指标，衡量回归预测的准确性。
- **RMSE (均方根误差)**：MSE 的平方根，与目标变量同单位，更直观地反映平均预测偏差。
- **MAE (平均绝对误差)**：对异常值不敏感，能反映模型在正常情况下的表现。
- **R² (决定系数)**：衡量模型解释了数据波动的多少比例，越接近 1 越好。

**2. 方向预测类（证明"涨跌"判得准）**：
- **Directional Accuracy (方向准确率)**：预测涨跌方向的准确率，计算公式为 $\text{DA} = \frac{1}{N}\sum_{i=1}^{N}\mathbb{I}[\text{sign}(\hat{y}_i) = \text{sign}(y_i)]$。这是金融预测的核心指标，直接回答"模型用来炒股，胜率是多少"。方向准确率超过 50% 表示模型具有预测能力。

**3. 量化投资类（证明"选股"能力强）** ⭐：
- **IC (Information Coefficient, 信息系数)**：预测值与真实值之间的 Pearson 相关系数，衡量模型预测的线性相关性。这是量化金融领域的黄金标准指标，直接回答"模型打分越高，股票是否真的涨得越好"。IC > 0.05 表示不错的结果，IC > 0.1 是非常好的结果。
- **RankIC (Rank Information Coefficient, 秩信息系数)**：预测值排名与真实值排名之间的 Spearman 秩相关系数，衡量模型的排序能力。比 IC 更稳健，不受个别暴涨暴跌股票的影响，是基金公司最看重的指标。RankIC > 0.05 表示不错的结果，RankIC > 0.1 是非常好的结果。
...
4. **评估指标专业化** ⭐：添加了量化金融领域的黄金标准指标（IC、RankIC），显著提升了论文的专业性和说服力。这些指标直接回答了"模型能否用于量化投资"这个关键问题，让评审专家（特别是懂金融的）更容易认可你的工作。
